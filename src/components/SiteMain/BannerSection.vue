<template>
  <div class="relative group">
    <div
      ...
      <img class="absolute" style="top: 65%;right: 125%;" src="./../../assets/img/slider/slider-shape-1.png"
        alt="shape">
      ...
      <img class="absolute" style="top: 12%;right: 20%;" src="./../../assets/img/slider/slider-shape-4.png" alt="shape">
    </div>
    <div class="h-screen w-full overflow-hidden flex flex-nowrap text-left relative z-0" id="slider">
      <div v-for="(banner, index) in banners" :key="index"
        class="flex-none w-full flex flex-row-reverse items-center justify-between px-14">
        <div>
        ...
        <div class="space-y-8 mb-16">
          ...
          <div> <router-link
              class="mt-20 bg-verde-custom hover:bg-indigo-900 text-white text-sm font-medium uppercase rounded-3xl px-14 py-3 fade-in-up delay-4"
              :to="{ name: 'products' }">{{ banner.buttonText }}</router-link>
          </div>
        </div>
      </div>
      <div
        class="absolute inset-x-0 bottom-10 flex justify-between px-14 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <button id="prevButton"
          class="bg-verde-custom opacity-50 hover:opacity-100 text-white rounded-full px-6 py-4 transition-opacity duration-300 ease-in-out">
